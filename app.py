...
def check_document_existance(user_doc_number): #tested
    doc_founded = False
...
def get_doc_owner_name(user_doc_number): #tested
    #user_doc_number = input('Введите номер документа - ') edited
    print()
...
def get_all_doc_owners_names(): #tested
    users_list = []
    for current_document in documents:
        try:
...
def remove_doc_from_shelf(doc_number): #nested tested
...
def add_new_shelf(shelf_number=''): #tested
    if not shelf_number:
...
def append_doc_to_shelf(doc_number, shelf_number): #nested tested
    add_new_shelf(shelf_number)
    directories[shelf_number].append(doc_number)


def delete_doc(user_doc_number): #tested
    #user_doc_number = input('Введите номер документа - ') edited
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in documents:
...
def get_doc_shelf(user_doc_number): #tested
    #user_doc_number = input('Введите номер документа - ') edited
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
...
def show_document_info(document): #tested
...
def add_new_doc(new_doc, new_doc_shelf_number): #tested
    #edited
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc_shelf_number = add_new_doc(new_doc, new_doc_shelf_number)
